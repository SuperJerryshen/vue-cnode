    ...
    <div class="user-detail">
      ...
      <div class="user-info">
        ...
        <div class="github">
          ...
          <a target="_blank"
             :href="'https://github.com/' + userDetailData.githubUsername">@{{ userDetailData.githubUsername }}</a>
        </div>
      ...
      <div class="topics">
        ...
        <ul class="topic-lists"
            v-if="userDetailData.recent_topics.length">
          <li v-for="(item, idx) in userDetailData.recent_topics"
              :key="idx">
            <article-card :article="item"
                          :simpleMode="true"></article-card>
          </li>
        </ul>
        <p class="no-content"
           v-if="!userDetailData.recent_topics.length">无话题</p>
      </div>
      <div class="replies">
        <h2 class="user-title">最近参与的话题</h2>
        <ul class="reply-lists"
            v-if="userDetailData.recent_topics.length">
          <li v-for="(item, idx) in userDetailData.recent_replies"
              :key="idx">
            <article-card :article="item"
                          :simpleMode="true"></article-card>
          </li>
