<template>
  <div class="reply">
    ...
    <ul class="comments">
      <li class="comment"
          v-for="(item, idx) in sortedComments"
          :key="idx">
        <div class="reply-author">
          <img @click="toAuthorDetail(item.author.loginname)"
               class="avatar"
        ...
        <div class="ups">
          <i class="iconfont icon-praise"
             @click="upReply(item.id)"
             :class="{'uped': item.is_uped}"></i>
...
export default {
  computed: {
    ...mapGetters(['userData', 'sortingMethod']),
    sortedComments() {
      // 根据sortingMethod的种类来进行排序
      if (this.sortingMethod === 'default') {
        return this.comments;
      }
      const copy = deepCopy(this.comments);
      if (this.sortingMethod === 'time') {
        return copy.sort((a, b) => new Date(b.create_at) - new Date(a.create_at));
      } else if (this.sortingMethod === 'hot') {
        return copy.sort((a, b) => b.ups.length - a.ups.length);
      }
    },
  },
  methods: {
