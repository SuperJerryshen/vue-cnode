<template>
  <page title="CNode中文社区"
        class="__page_home">
    <top-bar />
    <div class="content-tab-wrap">
      <div class="content-tab">
        <a @click.prevent="changeTab('all')"
           href="/"
           :class="{'selected': selectedTab === 'all'}">全部</a>
        <a @click.prevent="changeTab('good')"
           href="/"
           :class="{'selected': selectedTab === 'good'}">精华</a>
        <a @click.prevent="changeTab('share')"
           href="/"
           :class="{'selected': selectedTab === 'share'}">分享</a>
        <a @click.prevent="changeTab('ask')"
           href="/"
           :class="{'selected': selectedTab === 'ask'}">问答</a>
        <a @click.prevent="changeTab('job')"
           href="/"
           :class="{'selected': selectedTab === 'job'}">招聘</a>
        <a @click.prevent="changeTab('dev')"
           href="/"
           :class="{'selected': selectedTab === 'dev'}">客户端测试</a>
      </div>
    </div>
    <div class="content">
      <ul class="content-list">
        <li v-for="(item, idx) in articleLists"
            class="article"
            :key="idx">
...
export default {
  ...
  methods: {
    ...
    loadMoreData(tabType, pageCount) {
      ...
      this.axios
        .get(`https://cnodejs.org/api/v1/topics/?tab=${tabType}&page=${pageCount +
            1}`)
        .then(
          (res) => {
            this.$store.dispatch('delete_message');
  ...
  created() {
    ...
    // 就开始向服务器请求数据，并初始化。
    this.axios
      .get('https://cnodejs.org/api/v1/topics/?tab=all&page=1')
      .then((res) => {
